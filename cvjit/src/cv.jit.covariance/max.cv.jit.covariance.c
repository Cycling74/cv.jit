...
{	
	...
	setup((t_messlist **)&max_cv_jit_covariance_class, (method)max_cv_jit_covariance_new, (method)max_cv_jit_covariance_free, (short)sizeof(t_max_cv_jit_covariance), 
		0L, A_GIMME, 0);
...
void *max_cv_jit_covariance_new(t_symbol *s, long argc, t_atom *argv)
{
	...
	if (x=(t_max_cv_jit_covariance *)max_jit_obex_new(max_cv_jit_covariance_class,gensym("cv_jit_covariance"))) {
		if (o=jit_object_new(gensym("cv_jit_covariance"))) {
			...
			error("cv.jit.covariance: could not allocate object");

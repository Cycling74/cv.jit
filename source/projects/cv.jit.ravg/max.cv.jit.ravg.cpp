...
void *max_cv_jit_ravg_new(t_symbol *s, long argc, t_atom *argv);
void max_cv_jit_ravg_free(t_max_cv_jit_ravg *x);
void *max_cv_jit_ravg_class;
...
{	
	...
	q = jit_class_findbyname(gensym("cv_jit_ravg"));    
...
void *max_cv_jit_ravg_new(t_symbol *s, long argc, t_atom *argv)
{
	...
	if ((x=(t_max_cv_jit_ravg *)max_jit_obex_new(max_cv_jit_ravg_class,gensym("cv_jit_ravg")))) {
		if ((o= (t_jit_object*)jit_object_new(gensym("cv_jit_ravg")))) {
			...
			object_error((t_object*)x, "cv.jit.ravg: could not allocate object");

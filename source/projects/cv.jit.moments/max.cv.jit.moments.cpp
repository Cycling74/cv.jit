...
void max_cv_jit_moments_mproc(t_max_cv_jit_moments *x, void *mop)
{
		...
		jit_object_method(mop,_jit_sym_getoutputlist))))
	{
		...
		switch(ac) {
		case 1:
			outlet_float(x->massout,atom_getfloat(av));
			break;
		default:		
		...
		switch(ac) {
		case 1:
			outlet_float(x->centout,atom_getfloat(av));
			break;
		default:		
		...
		switch(ac) {
		case 1:
			outlet_float(x->huout,atom_getfloat(av));
			break;
		default:		
		...
		switch(ac) {
		case 1:
			outlet_float(x->momout,atom_getfloat(av));
			break;
		default:		
...
void *max_cv_jit_moments_new(t_symbol *s, long argc, t_atom *argv)
{
	...
	if ((x=(t_max_cv_jit_moments *)max_jit_obex_new(max_cv_jit_moments_class,gensym("cv_jit_moments")))) {
		if ((o= (t_jit_object*)jit_object_new(gensym("cv_jit_moments")))) {
			...
			object_error((t_object*)x, "cv.jit.moments: could not allocate object");

...
typedef struct _max_cv_jit_blobs_centroids 
{
	t_object		ob;
	void			*obex;
} t_max_cv_jit_blobs_centroids;

t_jit_err cv_jit_blobs_centroids_init(void); 

void *max_cv_jit_blobs_centroids_new(t_symbol *s, long argc, t_atom *argv);
void max_cv_jit_blobs_centroids_free(t_max_cv_jit_blobs_centroids *x);

void *max_cv_jit_blobs_centroids_class;
...
void *max_cv_jit_blobs_centroids_new(t_symbol *s, long argc, t_atom *argv)
{
	t_max_cv_jit_blobs_centroids *x;
	void *o;

	if (x=(t_max_cv_jit_blobs_centroids *)max_jit_obex_new(max_cv_jit_blobs_centroids_class,gensym("cv_jit_blobs_centroids"))) {
		if (o=jit_object_new(gensym("cv_jit_blobs_centroids"))) {
			max_jit_mop_setup_simple(x,o,argc,argv);			
			max_jit_attr_args(x,argc,argv);
		} else {
			error("cv.jit.blobs.centroids: could not allocate object");

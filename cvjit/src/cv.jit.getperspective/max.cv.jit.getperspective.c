...
typedef struct _max_cv_jit_getperspective {
	t_object	ob;
	void		*obex;
} t_max_cv_jit_getperspective;


// prototypes
BEGIN_USING_C_LINKAGE
t_jit_err	cv_jit_getperspective_init(void);
void		*max_cv_jit_getperspective_new(t_symbol *s, long argc, t_atom *argv);
void		max_cv_jit_getperspective_free(t_max_cv_jit_getperspective *x);
END_USING_C_LINKAGE

// globals
static void	*max_cv_jit_getperspective_class = NULL;
...
{	
	...
	cv_jit_getperspective_init();	
	setup((t_messlist**)&max_cv_jit_getperspective_class, (method)max_cv_jit_getperspective_new, (method)max_cv_jit_getperspective_free, sizeof(t_max_cv_jit_getperspective), 0, A_GIMME, 0);
	
	p = max_jit_classex_setup(calcoffset(t_max_cv_jit_getperspective, obex));
	q = jit_class_findbyname(gensym("cv_jit_getperspective"));    
...
void *max_cv_jit_getperspective_new(t_symbol *s, long argc, t_atom *argv)
{
	...
	if (x) {
		...
		if (o) {
			...
			jit_object_error((t_object*)x, "cv.jit.getperspective : could not allocate object");

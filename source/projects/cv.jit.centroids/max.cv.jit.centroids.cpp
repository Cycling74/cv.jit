...
{	
    ...
    max_jit_classex_mop_mproc(p, q, (void*)max_cv_jit_centroids_mproc); 	//custom mproc
    max_jit_classex_standard_wrap(p,q,0); 	
 	addmess((method)max_cv_jit_centroids_assist,			"assist",			A_CANT,0);
 	addmess((method)max_cv_jit_centroids_bang, "bang", 0);
	...
	if (max_jit_mop_getoutputmode(x)&&x->av) 
	{
		...
		else 
		{
			if (x->av->a_type==A_FLOAT)
				...
				outlet_int(x->massout,atom_getlong(x->av));
		}
		...
		{
			if (x->av->a_type==A_FLOAT)
				outlet_float(x->centroidsout, atom_getfloat(x->av));
			else 
				outlet_int(x->centroidsout, atom_getlong(x->av));
		}
	}
}

...
typedef struct _max_cv_jit_blobs_direction 
{
	t_object		ob;
	void			*obex;
} t_max_cv_jit_blobs_direction;

t_jit_err cv_jit_blobs_direction_init(void); 

void *max_cv_jit_blobs_direction_new(t_symbol *s, long argc, t_atom *argv);
void max_cv_jit_blobs_direction_free(t_max_cv_jit_blobs_direction *x);

void *max_cv_jit_blobs_direction_class;
...
{	
	...
	p = max_jit_classex_setup(calcoffset(t_max_cv_jit_blobs_direction,obex));	//Setup Max class
	q = jit_class_findbyname(gensym("cv_jit_blobs_direction"));				//Get a pointer to the Jitter object class
...
void *max_cv_jit_blobs_direction_new(t_symbol *s, long argc, t_atom *argv)
{
	t_max_cv_jit_blobs_direction *x;
	void *o;

	if ((x=(t_max_cv_jit_blobs_direction *)max_jit_obex_new(max_cv_jit_blobs_direction_class,gensym("cv_jit_blobs_direction")))) {
		if ((o= (t_jit_object*)jit_object_new(gensym("cv_jit_blobs_direction")))) {
			max_jit_mop_setup_simple(x,o,argc,argv);			
			max_jit_attr_args(x,argc,argv);
		} else {
			object_error((t_object*)x, "cv.jit.blobs.direction: could not allocate object");

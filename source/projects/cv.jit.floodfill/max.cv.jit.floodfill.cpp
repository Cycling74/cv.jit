...
typedef struct _max_cv_jit_floodfill 
{
	t_object		ob;
	void			*obex;
} t_max_cv_jit_floodfill;

t_jit_err cv_jit_floodfill_init(void); 

void *max_cv_jit_floodfill_new(t_symbol *s, long argc, t_atom *argv);
void max_cv_jit_floodfill_free(t_max_cv_jit_floodfill *x);
void *max_cv_jit_floodfill_class;
...
{	
	...
	q = jit_class_findbyname(gensym("cv_jit_floodfill"));    
...
void *max_cv_jit_floodfill_new(t_symbol *s, long argc, t_atom *argv)
{
	...
	if ((x=(t_max_cv_jit_floodfill *)max_jit_obex_new(max_cv_jit_floodfill_class,gensym("cv_jit_floodfill")))) {
		if ((o= (t_jit_object*)jit_object_new(gensym("cv_jit_floodfill")))) {
			...
			object_error((t_object*)x, "cv.jit.floodfill: could not allocate object");

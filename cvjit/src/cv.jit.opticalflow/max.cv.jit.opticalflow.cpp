...
typedef struct _max_cv_jit_opticalflow 
{
	t_object		ob;
	void			*obex;
} t_max_cv_jit_opticalflow;
...
{	
	...
	q = jit_class_findbyname(gensym("cv_jit_opticalflow"));    
...
void *max_cv_jit_opticalflow_new(t_symbol *s, long argc, t_atom *argv)
{
	t_max_cv_jit_opticalflow *x;
	void *o;

	if (x=(t_max_cv_jit_opticalflow *)max_jit_obex_new(max_cv_jit_opticalflow_class,gensym("cv_jit_opticalflow"))) {
		if (o=jit_object_new(gensym("cv_jit_opticalflow"))) {
			max_jit_mop_setup_simple(x,o,argc,argv);			
			max_jit_attr_args(x,argc,argv);
		} else {
			error("cv.jit.opticalflow: could not allocate object");

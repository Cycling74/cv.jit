...
{	
    ...
    max_jit_classex_mop_mproc(p, q, (void*)max_cv_jit_mass_mproc); 	//custom mproc
    max_jit_classex_standard_wrap(p,q,0); 	
 	addmess((method)max_cv_jit_mass_assist,			"assist",			A_CANT,0);
 	addmess((method)max_cv_jit_mass_bang, "bang", 0);
...
void *max_cv_jit_mass_new(t_symbol *s, long argc, t_atom *argv)
{
	...
	if ((x=(t_max_cv_jit_mass*)max_jit_obex_new(max_cv_jit_mass_class,gensym("cv_jit_mass")))) {
		if ((o= (t_jit_object*)jit_object_new(gensym("cv_jit_mass")))) {
			...
			object_error((t_object*)x, "cv.jit.mass: could not allocate object");

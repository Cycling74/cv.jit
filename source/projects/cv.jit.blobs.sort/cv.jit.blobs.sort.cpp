...
t_jit_err cv_jit_blobs_sort_init(void) 
{
	...
	attr = (t_jit_object*)jit_object_new(	_jit_sym_jit_attr_offset,"threshold",_jit_sym_float32,attrflags,(method)0L,(method)0L,calcoffset(t_cv_jit_blobs_sort,threshold));			
...
t_jit_err cv_jit_blobs_sort_matrix_calc(t_cv_jit_blobs_sort *x, void *inputs, void *outputs)
{
	...
	if (x&&in_matrix&&out_matrix) 
	{
		...
		if(in_minfo.dimcount != 1)
		{
			...
			object_error((t_object*)x, "Make sure object is connected to leftmost outlet of cv.jit.blobs.centroids!");
			goto out;
		}
		if((in_minfo.planecount != 3)&&(in_minfo.planecount != 17))
		{
			err = JIT_ERR_MISMATCH_PLANE;
			object_error((t_object*)x, "Make sure object is connected to leftmost outlet of cv.jit.blobs.centroids or cv.jit.blobs.moments!");
			goto out;
		}

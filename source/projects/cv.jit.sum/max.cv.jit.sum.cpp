...
{	
    ...
    max_jit_classex_mop_mproc(p, q, (void*)max_cv_jit_sum_mproc); 	//custom mproc
    max_jit_classex_standard_wrap(p,q,0); 	
 	addmess((method)max_cv_jit_sum_assist,			"assist",			A_CANT,0);
 	addmess((method)max_cv_jit_sum_bang, "bang", 0);
	...
	if (max_jit_mop_getoutputmode(x)&&x->av) 
	{
		...
		else 
		{
			if (x->av->a_type==A_FLOAT)
				outlet_float(x->sumout, atom_getfloat(x->av));
			else 
				outlet_int(x->sumout, atom_getlong(x->av));
		}
	}
}
...
void *max_cv_jit_sum_new(t_symbol *s, long argc, t_atom *argv)
{
	...
	if ((x=(t_max_cv_jit_sum *)max_jit_obex_new(max_cv_jit_sum_class,gensym("cv_jit_sum")))) {
		if ((o= (t_jit_object*)jit_object_new(gensym("cv_jit_sum")))) {
			...
			object_error((t_object*)x, "cv.jit.sum: could not allocate object");

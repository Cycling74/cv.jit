...
{	
	...
	cv_jit_lines_init();	
	setup((t_messlist **)&max_cv_jit_lines_class, (method)max_cv_jit_lines_new, (method)max_cv_jit_lines_free, (short)sizeof(t_max_cv_jit_lines), 
		0L, A_GIMME, 0);
...
void *max_cv_jit_lines_new(t_symbol *s, long argc, t_atom *argv)
{
	...
	if (x=(t_max_cv_jit_lines *)max_jit_obex_new(max_cv_jit_lines_class,gensym("cv_jit_lines"))) {
		if (o=jit_object_new(gensym("cv_jit_lines"))) {
			max_jit_mop_setup_simple(x,o,argc,argv);			
			max_jit_attr_args(x,argc,argv);
		} else {
			error("cv.jit.lines: could not allocate object");
